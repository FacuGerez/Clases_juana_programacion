...
import ar.edu.ort.tp1.examen.clases.EnvioPirvadoExpress;
...
public class Test {
	...
	private static Pedido crearPedidoNormal() {
		...
		p.agregarItem(buscarPorId(ID_HELADITO), 2);
		p.agregarItem(buscarPorId(ID_PATITAS_ORT), 3);
		p.agregarItem(buscarPorId(ID_PATITAS_ORT), 1);
		p.agregarItem(buscarPorId(ID_PATITAS_ORT), 2);
	...
	private static Pedido crearPedidoPrivadoExpress() {
		System.out.println("Creando pedido privado----");
		//TODO reemplazar los "..." con lo que corresponda
		Pedido p = new Pedido(new EnvioPirvadoExpress(), 20);
		p.agregarItem(buscarPorId(ID_PATITAS_ORT), 3);
